...
const EditPenalties = () => {
    ...
    useEffect(() => {
        ...
        const fetchDataById = async() => {
            try {
                const response = await API.get(`/penalties/${id}?token=${token}`);
                const getData = response.data.penalties;
                setPenaltiesName(getData.penalties_name);
                setDesc(getData.description);
                setCar(getData.no_car);
                setPenaltiesTotal(getData.penalties_total);
            } catch (error) {
                console.error(error);
            }
        }
    ...
    const handleRegister = async() => {
        try {
            const response = await API.put(`/penalties/${id}?token=${token}`, {
                penalties_name,
                description,
                no_car,
                penalties_total
    ...
    <div className='w-full min-h-screen py-16 flex justify-center items-center'>
        <div className="w-[850px] min-h-[600px] bg-white p-12 rounded-md">
            <div className="flex items-center gap-4">
                ...
                <div className="">
                    <h1 className='font-poppins-semibold text-[26px] text-primary'>Edit Penalties</h1>
            ...
            <div className="flex justify-center items-start mt-10 gap-24">
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <button onClick={handleRegister} className='w-[300px] bg-primary h-[55px] mt-6 cursor-pointer border-3 border-primary hover:bg-transparent hover:text-primary duration-200 rounded-md text-white mx-auto font-poppins-medium text-[16px]'>Edit Penalties</button>
                </div>
            </div>
        </div>
    </div>
