...
const EditRent = () => {
    ...
    useEffect(() => {
        ...
        const fetchById = async() => {
            try {
                const response = await API.get(`/rent/${id}?token=${token}`);
                const getData = response.data.rent;
                setTenant(getData.tenant);
                setCar(getData.no_car);
                setDateBorrow(getData.date_borrow);
                setDateReturn(getData.date_return);
                setDownPayment(getData.down_payment);
        ...
        fetchById();
        totalPayment();
    }, [token, down_payment, discount, id]);

    const handleRegister = async() => {
        try {
            const response = await API.put(`/rent/${id}?token=${token}`, {
                tenant,
                no_car,
                date_borrow,
                date_return,
    ...
    <div className='w-full min-h-screen py-16 flex justify-center items-center'>
        <div className="w-[850px] min-h-[600px] bg-white p-12 rounded-md">
            <div className="flex items-center gap-4">
                ...
                <div className="">
                    <h1 className='font-poppins-semibold text-[26px] text-primary'>Edit Rent</h1>
            ...
            <div className="flex justify-center items-start mt-10 gap-24">
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <button onClick={handleRegister} className='w-[300px] bg-primary h-[55px] mt-6 cursor-pointer border-3 border-primary hover:bg-transparent hover:text-primary duration-200 rounded-md text-white mx-auto font-poppins-medium text-[16px]'>Edit Rent</button>
                </div>
            </div>
        </div>
    </div>
