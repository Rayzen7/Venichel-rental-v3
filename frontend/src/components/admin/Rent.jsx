...
const Rent = () => {
    ...
    const handleEdit = async(id) => {
        navigate(`/admin/rent/edit/${id}`);
    }

    const handleDelete = async(id) => {
        try {
            const response = await API.delete(`/rent/${id}?token=${token}`);
            Swal.fire({
        ...
        <div className="bg-white w-[90%] min-h-[50vh] p-12 rounded-md">
            <div className=" font-poppins-semibold justify-between flex items-center mx-10">
                ...
                <Button
                    ...
                    width={26}
                    height={26}
                    onClick={() => navigate('/admin/rent/add')}
                />
            </div>
            {data.length > 0 ? (
                <table className='mt-16 border-collapse mx-auto'>
                    <thead>
                        <tr className='font-poppins-medium text-[16px] text-center'>
                            ...
                            <td className='px-6 border-b-3 pb-3 border-black'>Down Payment</td>
                    ...
                    {data.map((datas, index) => (
                        <tbody key={index}>
                            <tr className='font-poppins-medium text-[14px] text-center'>
                                ...
                                <td className='px-6 py-6'>{rupiahFormat(datas.down_payment)}</td>
